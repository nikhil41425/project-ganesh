...
type DonationItemForm = z.infer<typeof donationItemSchema>
...
export default function Donations({
  ...
  const form = useForm<DonationItemForm>({
    resolver: zodResolver(donationItemSchema),
    defaultValues: {
      name: '',
      amount: undefined,
      paid: undefined,
  ...
  const handleAddSubmit = async (data: DonationItemForm) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-2 md:gap-4">
        <div className="rounded-xl bg-gradient-to-r from-violet-600 to-purple-700 p-3 text-white shadow-lg md:p-6">
          <h3 className="text-xs font-medium opacity-90 md:text-sm">Donations (విరాళాలు)</h3>
          <p className="text-sm font-bold md:text-2xl">{items.length}</p>
        </div>
        <div className="bg-gradient-to-r from-orange-500 to-red-500 rounded-xl p-3 md:p-6 text-white shadow-lg">
      ...
      {canManage && showAddForm && (
        <div className="bg-white input-visible rounded-lg p-6 shadow-lg border border-white/20">
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row sm:items-end gap-3 sm:col-span-2 lg:col-span-3 xl:col-span-5">
      ...
      <div className="bg-white/80 backdrop-blur-lg rounded-xl shadow-lg border border-white/20 overflow-hidden">
        ...
        <div className="hidden md:block">
          <div className="overflow-x-auto">
            <table className="payment-table w-full min-w-[1000px]">
                  ...
                  <tr>
                    <td colSpan={9} className="px-4 py-12 text-center text-gray-500 text-sm">
                      <div className="flex flex-col items-center space-y-3">
                        ...
                        <p className="font-medium">No donations added yet</p>
                        <p className="text-xs text-gray-400">Click &quot;Add Donation&quot; to get started</p>
                      </div>
                    </td>
                  </tr>
                ) : (
                      ...
                      <tr key={item.id} className={`transition-colors duration-200 ${getStatusRowClass(item)}`}>
                        ...
                        <td className="px-3 py-4 text-sm font-semibold text-gray-900 border-r border-gray-200">
                          {isEditing ? (
                            ...
                            <div className="text-orange-600 font-bold">₹{item.amount}</div>
                          )}
                        </td>
                        <td className="px-3 py-4 text-sm font-semibold text-gray-900 border-r border-gray-200">
                          {isEditing ? (
                      ...
                      <tr key={item.id} className={`transition-colors duration-200 ${getStatusRowClass(item)}`}>
                        ...
                        <td className="px-2 py-3 text-sm border-r border-gray-200">
                          {isEditing ? (
                            ...
                            <div className="font-semibold text-orange-600">{formatCurrency(item.amount)}</div>
                          )}
                        </td>
                        <td className="px-2 py-3 text-sm border-r border-gray-200">
                          {isEditing ? (
