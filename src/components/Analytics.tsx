...
export default function Analytics({ auctionItems, membershipItems, spentItems, donationItems, duesItems, year }: AnalyticsProps) {
  ...
  return (
    <section className="rounded-3xl bg-[#071b25] p-4 text-slate-100 shadow-2xl sm:p-7 lg:p-10">
      <div className="mb-7">
      ...
      <div className="space-y-5">
        <article className="overflow-hidden rounded-2xl border border-slate-600/60 bg-[#102a36] shadow-xl">
          ...
          <div className="px-5 py-5">
            ...
            <p className={`mt-1 text-4xl font-black ${dashboard.accountBalance >= 0 ? 'text-cyan-400' : 'text-rose-400'}`}>{formatCurrency(dashboard.accountBalance)}</p>
          </div>
          <div className="grid border-y border-slate-600/60 sm:grid-cols-2 lg:grid-cols-4">
            {[
              ...
              <div key={String(label)} className="border-b border-slate-600/60 p-5 last:border-b-0 sm:border-r sm:[&:nth-child(2)]:border-r-0 lg:border-b-0 lg:[&:nth-child(2)]:border-r lg:last:border-r-0">
                <p className="min-h-10 text-sm text-slate-400">{label}</p>
                <p className={`text-2xl font-black ${color}`}>{formatCurrency(Number(value))}</p>
              </div>
            ))}
          </div>
          <p className="px-5 py-4 text-sm text-slate-400">Account Balance = {formatCurrency(dashboard.incomingPaid)} collected − {formatCurrency(dashboard.expensePaid)} paid expenses</p>
        </article>

        <article className="rounded-2xl border border-slate-600/60 bg-[#102a36] p-5 shadow-xl">
