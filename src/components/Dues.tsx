...
type DuesItemForm = z.infer<typeof duesItemSchema>
...
interface DuesProps {
  ...
  onAddItem: (data: DuesItemForm) => Promise<void>
  onDeleteItem: (id: string) => Promise<void>
  onUpdateItem: (id: string, data: Partial<DuesItem>) => Promise<void>
}
...
export default function Dues({
  ...
  const form = useForm<DuesItemForm>({
    resolver: zodResolver(duesItemSchema),
    defaultValues: {
      name: '',
      amount: undefined,
      paid: undefined,
  ...
  const startEditing = (item: DuesItem) => {
  ...
  const handleAddSubmit = async (data: DuesItemForm) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-2 md:gap-4">
        <div className="bg-gradient-to-r from-teal-500 to-cyan-500 rounded-xl p-3 md:p-6 text-white shadow-lg">
        ...
        <div className="bg-gradient-to-r from-red-500 to-pink-500 rounded-xl p-3 md:p-6 text-white shadow-lg">
        ...
        <div className="bg-white input-visible rounded-lg p-6 shadow-lg border border-white/20">
          ...
          >
            <div>
              ...
              <input
                {...form.register('name')}
                className="w-full px-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white input-visible shadow-sm transition-all duration-200"
            ...
            <div>
              ...
              <input
                ...
                placeholder="0"
              />
              {form.formState.errors.paid && (
                <p className="text-red-500 text-xs mt-1">{form.formState.errors.paid.message}</p>
              )}
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex gap-2">
                <button
                  type="submit"
                  className="flex-1 bg-gradient-to-r from-green-500 to-emerald-500 text-white px-4 py-3 rounded-lg hover:from-green-600 hover:to-emerald-600 transition-all duration-200 text-sm font-medium shadow-lg hover:shadow-xl transform hover:scale-105"
                >
                  Add
                </button>
                <button
                  ...
                  className="flex-1 bg-gradient-to-r from-gray-500 to-gray-600 text-white px-4 py-3 rounded-lg hover:from-gray-600 hover:to-gray-700 transition-all duration-200 text-sm font-medium shadow-lg hover:shadow-xl transform hover:scale-105"
                >
                  Cancel
                </button>
              </div>
      ...
      <div className="bg-white/80 backdrop-blur-lg rounded-xl shadow-lg border border-white/20 overflow-hidden">
        ...
        <div className="hidden md:block">
          <div className="overflow-x-auto">
            <table className="w-full divide-y divide-gray-200/50 min-w-[1000px]">
                  ...
                  <tr>
                    <td colSpan={8} className="px-4 py-12 text-center text-gray-500 text-sm">
                      <div className="flex flex-col items-center space-y-3">
                        ...
                        <p className="font-medium">No dues added yet</p>
                        <p className="text-xs text-gray-400">Click &quot;Add Dues&quot; to get started</p>
                      </div>
                    </td>
                  </tr>
                ) : (
                      ...
                      <tr key={item.id} className="hover:bg-blue-50/50 transition-colors duration-200">
                        <td className="px-3 py-4 text-sm font-medium text-gray-900 border-r border-gray-200">
                          {isEditing ? (
                            <input
                              ...
                              className="w-full px-2 py-1 border border-gray-300 rounded text-sm bg-white input-visible focus:ring-1 focus:ring-blue-500 focus:border-transparent"
                            />
                          ) : (
                        ...
                        <td className="px-3 py-4 text-sm font-semibold text-gray-900 border-r border-gray-200">
                          {isEditing ? (
                            ...
                            <div className="text-teal-600 font-bold">₹{item.amount}</div>
                          )}
                        </td>
                        <td className="px-3 py-4 text-sm font-semibold text-gray-900 border-r border-gray-200">
                          {isEditing ? (
        ...
        <div className="md:hidden">
          {items.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full divide-y divide-gray-200/50 min-w-[650px]">
                <thead className="bg-gradient-to-r from-gray-50/80 to-gray-100/80 backdrop-blur-sm">
                  <tr>
                      ...
                      <tr key={item.id} className="hover:bg-blue-50/50 transition-colors duration-200">
                        <td className="px-2 py-3 text-sm border-r border-gray-200">
                          {isEditing ? (
                            <input
                              ...
                              value={editFormData?.name || ''}
                              onChange={(e) => handleEditFormChange('name', e.target.value)}
                              className="w-full px-1 py-1 border border-gray-300 rounded text-xs bg-white input-visible focus:ring-1 focus:ring-blue-500 focus:border-transparent"
                            />
                          ) : (
                        ...
                        <td className="px-2 py-3 text-sm border-r border-gray-200">
                          {isEditing ? (
                            ...
                            <div className="font-semibold text-teal-600">{formatCurrency(item.amount)}</div>
                          )}
                        </td>
                        <td className="px-2 py-3 text-sm border-r border-gray-200">
                          {isEditing ? (
