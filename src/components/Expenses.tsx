...
import { z } from 'zod'
import { Trash2, Edit2, Save, X, Plus, Search, Receipt } from 'lucide-react'
...
// Form schema
const spentItemSchema = z.object({
...
type SpentItemForm = z.infer<typeof spentItemSchema>
...
interface ExpensesProps {
  ...
  onAddItem: (data: SpentItemForm) => Promise<void>
  onDeleteItem: (id: string) => Promise<void>
  onUpdateItem: (id: string, data: Partial<SpentItem>) => Promise<void>
}
...
export default function Expenses({
  ...
  const form = useForm<SpentItemForm>({
    resolver: zodResolver(spentItemSchema),
    defaultValues: {
      item: '',
      amount: undefined,
      paid: undefined,
  ...
  const startEditing = (item: SpentItem) => {
  ...
  const handleAddSubmit = async (data: SpentItemForm) => {
  ...
  const calculateSummary = () => {
    ...
    return { totalAmount, totalPaid, totalDue, totalExpenses }
  }

  const { totalAmount, totalPaid, totalDue, totalExpenses } = calculateSummary()
      ...
      <div className="bg-white/80 backdrop-blur-lg rounded-xl shadow-lg border border-white/20 overflow-hidden">
        ...
        <div className="hidden md:block">
          <div className="overflow-x-auto">
            <table className="w-full divide-y divide-gray-200/50 min-w-[910px]">
              <thead className="bg-gradient-to-r from-gray-50/80 to-gray-100/80 backdrop-blur-sm">
                <tr>
                  <th className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider min-w-[150px] border-r border-gray-200">Item</th>
              ...
              <tbody className="bg-white/60 backdrop-blur-sm divide-y divide-gray-200/30">
                {items.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="px-4 py-12 text-center text-gray-500 text-sm">
                      <div className="flex flex-col items-center space-y-3">
                        ...
                        <p className="font-medium">No expenses added yet</p>
                        <p className="text-xs text-gray-400">Click &quot;Add Expense&quot; to get started</p>
                      </div>
                    </td>
                  </tr>
                ) : (
                  items.map((item) => {
                    ...
                    return (
                      <tr key={item.id} className="hover:bg-green-50/50 transition-colors duration-200">
                        <td className="px-3 py-4 text-sm font-medium text-gray-900 border-r border-gray-200">
                          {isEditing ? (
                            ...
                            <div className="truncate font-semibold" title={item.item}>{item.item}</div>
                          )}
                        </td>
                        <td className="px-3 py-4 text-sm font-semibold text-gray-900 border-r border-gray-200">
                          {isEditing ? (
                            ...
                            <div className="text-green-600 font-bold">₹{item.amount}</div>
                          )}
                        </td>
                        <td className="px-3 py-4 text-sm font-semibold text-gray-900 border-r border-gray-200">
                          {isEditing ? (
                            ...
                            <div className="text-blue-600 font-bold">₹{item.paid}</div>
                          )}
                        </td>
                        <td className="px-3 py-4 text-sm font-semibold text-gray-900 border-r border-gray-200">
          ...
          {items.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full divide-y divide-gray-200/50 min-w-[650px]">
                <thead className="bg-gradient-to-r from-gray-50/80 to-gray-100/80 backdrop-blur-sm">
                  <tr>
                    <th className="px-2 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider min-w-[120px] border-r border-gray-200">Item</th>
                      ...
                      <tr key={item.id} className="hover:bg-blue-50/50 transition-colors duration-200">
                        <td className="px-2 py-3 text-sm border-r border-gray-200">
                          {isEditing ? (
                            ...
                            <div className="font-medium text-gray-900 truncate" title={item.item}>{item.item}</div>
                          )}
                        </td>
                        <td className="px-2 py-3 text-sm border-r border-gray-200">
                          {isEditing ? (
                        ...
                        <td className="px-2 py-3 text-sm border-r border-gray-200">
                          {isEditing ? (
                            ...
                            <div className="font-semibold text-blue-600">{formatCurrency(item.paid)}</div>
                          )}
                        </td>
                        <td className="px-2 py-3 text-sm border-r border-gray-200">
